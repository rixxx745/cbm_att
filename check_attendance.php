			...
			<div class="card-body">
				<form id="manage-attendance">
					...
					<div class="row justify-content-center">
						<label for="" class="mt-2">Class per Courses</label>
						<div class="col-sm-4">
				            <select name="class_course_id" id="class_course_id" class="custom-select select2 input-sm">
				                <option value=""></option>
				                <?php
				                $class = $conn->query("SELECT cs.*,concat(co.program,' ',c.level,'-',c.section) as `class`,s.course,f.faculty_name as faculty_name FROM class_course cs inner join `class` c on c.id = cs.class_id inner join programs co on co.id = c.program_id inner join faculty f on f.id = cs.faculty_id inner join courses s on s.id = cs.course_id ".($_SESSION['login_faculty_id'] ? " where f.id = {$_SESSION['login_faculty_id']} ":"")." order by concat(co.program,' ',c.level,'-',c.section) asc");
				                while($row=$class->fetch_assoc()):
				                ?>
				                <option value="<?php echo $row['id'] ?>" data-cid="<?php echo $row['id'] ?>" <?php echo isset($class_course_id) && $class_course_id == $row['id'] ? 'selected' : (isset($class_course_id) && $class_course_id == $row['id'] ? 'selected' :'') ?>><?php echo $row['class'].' '.$row['course']. ' [ '.$row['faculty_name'].' ]' ?></option>
				                <?php endwhile; ?>
				            </select>
						</div>
						<div class="col-sm-3">
...
$(document).ready(function(){
	if('<?php echo isset($class_course_id) ? 1 : 0 ?>' == 1){
		start_load()
		$.ajax({
			url:'ajax.php?action=get_class_list',
			method:'POST',
			data:{class_course_id:$('#class_course_id').val(),doc:$('#doc').val(),att_id :'<?php echo isset($id) ? $id : '' ?>' },
				...
				if(resp){
					...
					if(Object.keys(resp).length > 0){
						...
						Object.keys(resp.data).map(function(k){
							var name = resp.data[k].student_name;
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					  ...
					  alert_toast("Class already has an attendance record with the selected course and date.",'danger')
					  end_load();
				}
			}
		})
