...
<div class="container-fluid">
	<form action="" id="manage-faculty">
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" name="fuser_no"  value="<?php echo isset($fuser_no) ? $fuser_no :'' ?>" required>
		</div>
		<div class="form-group">
			<label for="" class="control-label">Name</label>
			<input type="text" class="form-control" name="faculty_name"  value="<?php echo isset($faculty_name) ? $faculty_name :'' ?>" required>
		</div>
		<div class="form-group">
	...
	$('#manage-faculty').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				...
				$('#msg').html('<div class="alert alert-danger mx-2">User number already exist.</div>')
				end_load()
				}	
			}
		})
