					...
					<div class="card-body">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$student = $conn->query("SELECT s.*,concat(co.program,' ',c.level,'-',c.section) as `class` FROM students s inner join `class` c on c.id = s.class_id inner join programs co on co.id = c.program_id order by s.student_name desc ");
								while($row=$student->fetch_assoc()):
								?>
								<tr>
									<td class="text-center"><?php echo $i++ ?></td>
									<td>
										<p> <b><?php echo $row['suser_no'] ?></b></p>
									</td>
									<td>
...
<script>
	...
	$('.delete_student').click(function(){
		_conf("Are you sure you want to delete this student?","delete_student",[$(this).attr('data-id')])
	})
