...
<div class="container-fluid">
	<div class="col-lg-12"><br>
		<div class="card">
			<div class="card-header"><b>Attendance Overview</b></div>
			<div class="card-body">
					...
					<div class="row justify-content-center">
						...
						<div class="col-sm-4">
				            <select name="class_course_id" id="class_course_id" class="custom-select select2 input-sm">
				                ...
				                ?>
				                <option value="<?php echo $row['id'] ?>" data-cid="<?php echo $row['id'] ?>" <?php echo isset($class_course_id) && $class_course_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['class'].' '.$row['course']. ' [ '.$row['faculty_name'].' ]' ?></option>
				                <?php endwhile; ?>
				            </select>
						</div>
...
<script>
	...
	$('#filter').click(function(){
		...
		$.ajax({
			...
			method:'POST',
			data:{class_course_id:$('#class_course_id').val(), doc:$('#doc').val()},
			success:function(resp){
				if(resp){
					...
					$('#att-list').html('')
					_table.find('.program').text(!!resp.details.program ? resp.details.program : '')
